...
namespace internal {
    ...
    // This handles lambdas, functors, and function pointers, stripping const/noexcept/etc.
    template <typename T>
...
requires requires {
    ...
    requires requires(typename internal::function_traits<F>::args_tuple t) {
        { absl::Hash<typename internal::function_traits<F>::args_tuple>{}(t) } -> std::convertible_to<size_t>;
    };
}
auto automemoize(F f) {
    ...
    return [f, cache = absl::flat_hash_map<ArgsTuple, ReturnType>()](auto&&... args) mutable -> ReturnType {
